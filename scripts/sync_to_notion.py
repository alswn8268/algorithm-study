...
SECTION_KEYWORDS = ["문제 풀이", "모의 코딩테스트", "모의코딩테스트"]
...
def find_code_block(page_id, day_korean):
    ...
    for block in blocks:
        ...
        if btype == "heading_2":
            ...
            if day_korean in text and any(kw in text for kw in SECTION_KEYWORDS):
                in_day_section = True
                in_member_section = False
        ...
        elif in_day_section and btype == "heading_3":
            text = "".join(
                t.get("plain_text", "")
        ...
        elif in_day_section and in_member_section and btype == "code":
            code_text = "".join(
                t.get("plain_text", "")
                for t in block["code"].get("rich_text", [])
            ).strip()
            # 비어 있거나 주석 placeholder인 코드 블록은 건너뜀
            if not code_text or code_text.startswith("#") and len(code_text) < 30:
                print(f"  placeholder 블록 건너뜀: {block['id']}")
                continue
...
def update_code_block(block_id, code, language="python"):
    ...
    # Notion rich_text 최대 2000자 제한 처리
